...
def get_paginated_questions(offset, limit):
    cursor = g.db.cursor()
    cursor.execute("""
        SELECT q.id, q.title, q.time, COUNT(a.id) as answer_count, p.username
        FROM question q
        LEFT JOIN answer a ON q.id = a.question_id
        LEFT JOIN passwd p ON q.user_id = p.id
        GROUP BY q.id, p.username
        ORDER BY q.time DESC
        LIMIT %s OFFSET %s
    """, (limit, offset))
    questions = cursor.fetchall()
...
# 질문 삽입
def insert_question(title, content, user_id):
    cursor = g.db.cursor()
    cursor.execute("INSERT INTO question (title, content, user_id) VALUES (%s, %s, %s)", (title, content, user_id))
    g.db.commit()
...
def insert_user(username, password, email):
    cursor = g.db.cursor()
    cursor.execute("INSERT INTO passwd (username, password, email) VALUES (%s, %s, %s)", (username, password, email))
    g.db.commit()
...
def search_questions(kw, offset, limit):
    ...
    search = f"%{kw}%"
    cursor.execute("""
        SELECT DISTINCT q.id, q.title, q.time, COUNT(a.id) as answer_count, p.username
        FROM question q
        LEFT JOIN answer a ON q.id = a.question_id
        LEFT JOIN passwd p ON q.user_id = p.id
